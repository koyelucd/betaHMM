...
#'    \item z - A matrix of dimension \eqn{C \times K} containing the conditional
#'     posterior probability of each CpG site belonging to each of the
#'      \eqn{K} hidden states.
#'    }
...
BaumWelch = function(data, trained_params, M, N, R, n.iter = 100, seed = NULL){
  ...
  for(i in 1:n.iter){
    ...
    for(r in 1:R) ## treatment groups
    {
      ...
      # }
      ### check this part
      if(N[r] == 1) ## if no replicates or analysing a single patient
      {
        y1[r,] <- apply((eta*(log(data[,n1:n2]))),
                     FUN=sum,MARGIN = 2)/(N[r]*apply(eta, FUN = sum,
                                                     MARGIN = 2))

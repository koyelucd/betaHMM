globalVariables(c("betaHMMresults","CHR","IlmnID","MAPINFO","chr","cond_prob_threshold"))
...
#' The default option has been set as "FALSE" due to package testing limitations.
...
#'\item treatment_group - The names of the treatment groups/conditions analysed.
                     ...
                     seed = NULL,...)
{
  ...
  {
    stop("Please provide a dataframe containing annotation for EPIC array.")
  }else
  if(M > 3 | M < 2)
  {
  ...
  if(R < 2)
  {
    stop("R cannot be < 2 to identify DMCs between 2 or more conditions.")
  }else
    if(N < 1)
    {
      stop("N cannot be less than 1 as one or more DNA
           replicates need to be analysed.")
    }
  ...
  final_subset <- subset(methylation_data, select = - IlmnID)
  # final_subset[final_subset == 1] <- NA
  # final_subset[final_subset==0]=NA
  #
  ...
  if(abs(min(final_subset) - 0) < 1e-6)
  {
   ...
   cols <- which(colnames(methylation_data) != "IlmnID")
   methylation_data[,cols] <- final_subset

  data <- merge(methylation_data,annotation_file[,c("IlmnID","CHR","MAPINFO")],
             by="IlmnID")
  ...
  col_order2 <- which(colnames(data) != "IlmnID" & colnames(data) != "CHR" &
                      colnames(data) != "MAPINFO")
  data <- data[ , c(col_order, col_order2)]
  K <- M ^ R
  C <- nrow(data)
  ##Function for checking if a variable is scalar or not
  is.scalar <- function(x) is.atomic(x) && length(x) == 1L && Im(x)==0

  ## Convert AUC and cond_prob
  if(is.scalar(N))
  {
    N <- rep(N,R)
  }else if(length(N) < R)
  ...
  ## Code to replace 1 with next maximum value and 0 with next minimum value
 #final_subset= subset(sorted_data, select = -c(CHR, MAPINFO, IlmnID))



  ## parallel code for each chromosome
  ...
  betaHMM_workflow<-function(data_chr, K, M, N, R, chr, seed = NULL)
  {
    ...
    # Remove non-numeric columns using subset
    ...
    ## Viterbi algorithm for hidden state estimation
  ...
  beta_chr_out <- foreach::foreach(chr = 1:length(chr_unique),
                                  .combine = 'comb',
                                  .multicombine = TRUE,
                                 ...
                                 uncertainty_threshold = 0.2,...)
{
  ...
  {
    stop("AUC threshold cannot be a vector with length less/greater than the no.
         of chromosomes.")
  }
  ...
  for(i in 1:length(chr_unique))
  {
    ...
    colnames(x) <- c("State", "AUC")
    x$AUC <- as.numeric(x$AUC)
    auc_list[[i]] <- x
    x_co <- x[x$AUC >= AUC_threshold[i], ]
    ...
    if(nrow(x_co)!=0)
    {
      ...
      df_chr$DMC <- 0
      uncertainty_list[[i]] <- 1
    }

    df_complete <- rbind(df_complete, df_chr)
  }
  df_complete <- df_complete[-1, ]

  rownames(df_complete) <- df_complete$IlmnID
  chromosome_number_list <- paste("chr", chr_unique)
  names(auc_list) <- chromosome_number_list
  names(uncertainty_list) <- chromosome_number_list
  select.results <- SummarizedExperiment(assays = df_complete,
                                         metadata = list(K = K, N = N, R = R,
                                                         treatment_group=
                                                         treatment_group,
                                                         AUC = auc_list,
...
#' chromosome.
#' @export
#' @param dmc_identification_object a \code{dmcResults} object or the assay data
#' from the dmcResults.
                                 ...
                                 parallel_process = FALSE,...)
{
  ...
  object <- dmc_identification_object
  dmc_df <- object
  # N=N(object)
  # R=R(object)
  # K=K(object)
  chr_unique <- unique(dmc_df$CHR)
  #mat_complete=c(NA,NA,NA)
  ...
  {
    #dmc_df_chr=dmc_df[dmc_df$CHR=="2",]
    ...
    # {dmc_df[dmc_df$IlmnID==x,"CHR"]})
    map_start <- sapply(df_unique$start_CpG, function(x)
    {dmc_df_chr[dmc_df_chr$IlmnID == x, "MAPINFO"]})
    ...
    for(j in 1:nrow(df_unique))
    {
      ...
      df_unique[j,"DMCs"] <- paste(x,collapse = ",")
    }
    return(df_unique)
  }

  dmr_mat <- foreach::foreach(chr = 1:length(chr_unique),
                              .combine = rbind) %dopar%
    dmr_parallel(dmc_df[dmc_df$CHR == chr_unique[chr], ],
                     DMC_count)
  ...
  select.results <- SummarizedExperiment(assays = dmr_df,
                                         metadata =
                                           list(chromosome_number = chr_unique))
